...
- `--sub-repo`는 `resolve_sha.py` 전용이고, integration checkout의 해당
...
- 동작 변경에는 해당 스크립트의 테스트(`tests/test_resolve.py`·
  `tests/test_predecessors.py`·`tests/test_viz.py`)에 회귀 테스트를
  추가하고 `python3 -m unittest discover -s tests`가 통과해야 한다.
